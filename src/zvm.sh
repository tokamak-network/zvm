...
function help () {
    echo "Commands"
    echo "1. Setup phase 1:          $ ./zvm.sh phase1"
    echo "2. Setup phase 2:          $ ./zvm.sh phase2 [circuit name]"
    echo "3. Debug with the witness: $ ./zvm.sh debug [circuit name] [input json file path]"
    echo "4. Generate a proof:       $ ./zvm.sh generate-proof [proof file name] [public file name]"
    echo "5. Verify a proof:         $ ./zvm.sh verify-proof [proof file path] [public file path] "
} 
...
# debug
# ./zvm.sh debug [circuit name] [input json file]
elif [ "$1" == "debug" -a $# -eq 3 ]
then
...
# generate proof
# ./zvm.sh generate-proof [proof file name] [public file name]
elif [ "$1" == "generate-proof" -a $# -eq 3 ]
then 
    snarkjs groth16 prove circuit_final.zkey witness.wtns $2.json $3.json

# verify proof
# ./zvm.sh verify-proof [proof file path] [public file path] 
